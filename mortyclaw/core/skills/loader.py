...
from ..tools.office import execute_office_shell
...
def load_dynamic_skills() -> List[StructuredTool]:
    ...
    for item in os.listdir(SKILLS_DIR):
        ...
        try:
                ...
                def runner(mode: str, command: str = "") -> str:
                    if mode == "help":
                        return (
                            ...
                            f"{md_content[:3000]}\n"
                            "====================================\n"
                            "提示：请根据以上说明，如果觉得能解决问题，就将 mode 设为 'run'，"
                            "并将拼装好的执行命令填入 command 重新调用。否则就尝试其他工具，"
                            "没有其他合适工具就老实跟用户说"
                        )
                    if mode == "run":
                        if not command:
            ...
            print(f" \033[38;5;196m[警告] 技能包 {item} 加载失败: {exc}\033[0m")
