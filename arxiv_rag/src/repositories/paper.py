...
class PaperRepository:
    ...
    def get_excess_papers(self, keep_latest: int, limit: Optional[int] = None) -> List[Paper]:
        stmt = select(Paper).order_by(Paper.published_date.desc()).offset(max(keep_latest, 0))
    ...
    def delete_by_ids(self, paper_ids: List[UUID]) -> int:
        if not paper_ids:
            return 0

        stmt = delete(Paper).where(Paper.id.in_(paper_ids))
