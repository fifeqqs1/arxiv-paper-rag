...
class AgenticRAGService:
    ...
    async def ask(
        ...
        """
        provider_to_use = self.graph_config.settings.resolve_llm_provider(provider)
        model_to_use = self.graph_config.settings.resolve_llm_model(provider_to_use, model or self.graph_config.model)

        logger.info("=" * 80)
        logger.info("Starting Agentic RAG Request")
        logger.info(f"Query: {query}")
        logger.info(f"User ID: {user_id}")
        logger.info(f"Provider: {provider_to_use}")
        ...
        async def _execute_with_trace():
            ...
            if trace is not None:
                with trace as trace_obj:
                    ...
                    return await self._run_workflow(query, provider_to_use, model_to_use, user_id, trace_obj)
            else:
                return await self._run_workflow(query, provider_to_use, model_to_use, user_id, None)
    ...
    async def _run_workflow(self, query: str, provider_to_use: str, model_to_use: str, user_id: str, trace) -> dict:
    ...
    def _extract_sources(self, result: dict) -> List[str]:
        ...
        for source in relevant_sources:
            if hasattr(source, "url") and getattr(source, "url"):
