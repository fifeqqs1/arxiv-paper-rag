...
class LangfuseTracer:
    ...
    def trace_rag_request(
        ...
        """
        Backward-compatible request trace wrapper for the simplified RAG tracer.

        When Langfuse is disabled or unavailable, this becomes a no-op context manager.
        """
        if not self.client:
            yield None
            return
        ...
        try:
            ...
            logger.error(f"Error creating request trace: {e}")
            yield None
        finally:
    ...
    def create_span(
        ...
        """
        Backward-compatible span creation helper used by RAG and agentic flows.
        """
        if not self.client:
            return None
