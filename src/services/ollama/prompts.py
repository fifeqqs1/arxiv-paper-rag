...
class RAGPromptBuilder:
    ...
    def create_rag_prompt(self, query: str, chunks: List[Dict[str, Any]]) -> str:
        ...
        prompt += (
            "### Answer:\n"
            "Please answer in Simplified Chinese by default unless the user explicitly asks for another language. "
            "Provide a natural, conversational response (not JSON) and cite sources using [arXiv:id] format. "
            "Use citations at paragraph or bullet level, not after every sentence; for a single-paper answer, "
            "cite once near the start and avoid repeating the same citation after every point.\n\n"
        )
