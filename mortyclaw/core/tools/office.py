...
def _is_within_dir(target_path: str, base_dir: str) -> bool:
    """使用 realpath + commonpath 进行稳健的目录包含校验。"""
    try:
...
def _get_safe_path(relative_path: str) -> str:
    """
    将模型传入的相对路径转换为 office 内绝对路径，并死死检查它是否越界！
    ...
    if not _is_within_dir(target_path, base_dir):
        raise PermissionError(f"越权拦截：你试图访问沙盒外的路径 '{relative_path}'！你只能在 office 工位内活动。")

    return target_path

def _get_read_path(path_value: str) -> str:
    """
    读取工具专用路径解析：
    - 相对路径：从 office 作为锚点解析，但不再限制必须留在 office 内
    - 绝对路径：允许只读访问任意目录/文件
    - 只读能力尽量放宽；写入、删除、shell 边界仍由其他工具单独控制
    """
    normalized = os.path.expanduser(path_value or "")
    if not normalized:
        return os.path.realpath(OFFICE_DIR)

    if os.path.isabs(normalized):
        return os.path.realpath(normalized)

    return os.path.realpath(os.path.join(OFFICE_DIR, normalized))

@mortyclaw_tool
def list_office_files(sub_dir: str = "") -> str:
    """
    查看文件和文件夹列表。
    - 如果传入相对路径，则从 office 工位作为锚点解析。
    - 如果传入绝对路径，也允许进行只读查看。
    - 只读查看不受 office 边界限制；写权限依然由其他工具单独约束。
    """
    try:
        target_dir = _get_read_path(sub_dir)
...
def read_office_file(filepath: str) -> str:
    ...
    - 如果传入相对路径，则从 office 工位作为锚点解析。
    - 如果传入绝对路径，也允许进行只读读取。
    - 只读读取不受 office 边界限制；写权限依然由其他工具单独约束。
    """
    try:
        target_path = _get_read_path(filepath)
